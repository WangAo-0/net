 ...
 * @LastEditors: oliver
 * @LastEditTime: 2024-01-04 17:45:43
 */
#include "MyDiagram.h"
#include <arpa/inet.h>
#include <chrono>
...
int startPressureGenrator() {
  ...
  std::vector<std::chrono::time_point<std::chrono::steady_clock>> startTimes(NUM_CLIENTS);
  ...
  for (int i = 0; i < NUM_CLIENTS; ++i) {
    ...
    // 客户端发送ID给服务器
    clientIDMap[clientSockets[i]] = i; // 将ID和客户端套接字的关系存储到unordered_map中
    ...
    while (true) {
      ...
      for (int i = 0; i < numEvents; ++i) {
        if (events[i].events & EPOLLOUT) {
          ...
          std::string data =
              "Hello, server! I'm client " + std::to_string(i) + ".";
          ...
          startTimes[events[i].data.fd - clientSockets[0]] = std::chrono::steady_clock::now();
